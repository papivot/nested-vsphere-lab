...
rollback_supervisor() {
  ...
  for id in $(vc_api GET "${VCSA_IP}" "$tok" "/api/content/library" 2>/dev/null | jq -r '.[]' 2>/dev/null); do
    ...
    if [[ "$name" == "${CONTENT_LIB}" ]]; then
      ...
      # Delete is per-type (the generic /api/content/library/{id} only supports
      # GET). Try subscribed then local so either library type is cleaned up.
      if curl -sk -X DELETE -H "vmware-api-session-id: ${tok}" \
           "https://${VCSA_IP}/api/content/subscribed-library/${id}" >/dev/null 2>&1 \
         || curl -sk -X DELETE -H "vmware-api-session-id: ${tok}" \
           "https://${VCSA_IP}/api/content/local-library/${id}" >/dev/null 2>&1; then
        ok "Content library removed."
      else
        warn "Could not remove content library."
      fi
      break
    fi
  done
